...
% 	labels(1,1) = magnitude;  %label for 0-shift (original sample)
	...
	[rs, cs] = ndgrid((1:sz(1)) - floor(sz(1)/2), (1:sz(2)) - floor(sz(2)/2)); % 使rs,cs有正有负
	labels = exp(-0.5 / sigma^2 * (rs.^2 + cs.^2));
    subplot(121)
    mesh(labels)
    title('labels without circulant')
	%move the peak to the top-left, with wrap-around  
	labels = circshift(labels,-floor(sz(1:2) / 2) + 1); % 【正式跟踪时，加入完美跟踪到目标，则目标的最大值也会出现在最左上角】
    subplot(122)
    mesh(labels)
    title('circul for [10 10]')
