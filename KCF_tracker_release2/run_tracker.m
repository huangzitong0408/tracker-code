...
function [precision, fps] = run_tracker(video, kernel_type, feature_type, show_visualization, show_plots)
	...
	base_path = './data/Benchmark';  % 【学习： ./ 代表当前路径】

	%default settings 【函数未传参数，值为0】
	...
	case 'hog'
		...
		features.hog = true;
		features.hog_orientations = 9;  % 【设置hog的方向（方向梯度直方图），9个方向，每个方向间隔40°】
		cell_size = 4; % 【进行hog提取时的区域大小】
...
% 【1图像文件、2第一帧位置、3目标size、4图像目标所在位置、5视频地址】 
%% 加载完视频信息，就进入到tracker，这里才真正进入到跟踪算法的主要部分，前面只是预备工作
		%call tracker function with all the relevant parameters
